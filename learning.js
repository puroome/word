...
import { imageDBCache } from "./cache.js";
...
export const learningMode = {
    ...
    async start() {
        ...
        if (!startWord) {
            try {
                const savedIndex = parseInt(localStorage.getItem(LOCAL_STORAGE_KEYS.LAST_INDEX) || '0');
    ...
    launchApp() {
        ...
        state.elements.progressBarContainer.classList.remove('hidden');
    ...
    async displayWord(index) {
        ...
        if (!wordData) {
         ...
         if (!this.state.isMistakeMode && !this.state.isFavoriteMode) {
            try {
                localStorage.setItem(LOCAL_STORAGE_KEYS.LAST_INDEX, index);
            } catch (e) {
    ...
    async startMistakeReview(mistakeWords) {
        ...
        if (this.state.currentWordList.length === 0) {
            ...
            if(window.appNavigateTo) window.appNavigateTo('selection');
            return;
        }
        this.launchApp();
    },
    async startFavoriteMode() {
        ...
        if(favoriteWords.length === 0) {
            ui.showToast("즐겨찾기에 등록된 단어가 없습니다.", true);
            if(window.appNavigateTo) window.appNavigateTo('selection');
            return;
        }
        const wordMap = new Map(state.wordList.map(wordObj => [wordObj.word, wordObj]));
    ...
    updateProgressBar(index) {
        ...
        if (total <= 1) {
            state.elements.progressBarFill.style.width = '100%';
            state.elements.progressBarHandle.style.left = '100%';
            if (state.elements.progressBarNumber) {
                state.elements.progressBarNumber.textContent = total > 0 ? '1' : '';
                state.elements.progressBarNumber.style.left = '100%';
            }
            return;
        }
        const percentage = (index / (total - 1)) * 100;
        state.elements.progressBarFill.style.width = `${percentage}%`;
        state.elements.progressBarHandle.style.left = `calc(${percentage}% - ${state.elements.progressBarHandle.offsetWidth / 2}px)`;
        if (state.elements.progressBarNumber) {
            state.elements.progressBarNumber.textContent = index + 1;
            state.elements.progressBarNumber.style.left = `${percentage}%`;
        }
    },
    handleProgressBarInteraction(e) {
        if (learningMode.elements.appContainer.classList.contains('hidden')) return;

        const track = state.elements.progressBarTrack;
    ...
    async toggleFavorite() {
        ...
        if (this.state.isFavoriteMode && !newStatus) {
            ...
            if (this.state.currentWordList.length === 0) {
                ui.showToast("즐겨찾기 목록이 비었습니다.", false);
                if(window.appNavigateTo) window.appNavigateTo('selection');
                return;
            }
    ...
    updateFavoriteIcon(isFavorite) {
        state.elements.favoriteIcon.classList.toggle('text-yellow-400', isFavorite);
        state.elements.favoriteIcon.classList.toggle('text-gray-400', !isFavorite);
        state.elements.favoriteIcon.classList.toggle('fill-current', isFavorite);
    }
};