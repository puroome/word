...
import { utils, correctBeep, incorrectBeep, playSequence } from "./utils.js";
...
export const quizMode = {
    ...
    async start(quizType) {
        ...
        // main.js에서 할당된 navigateTo 호출
        if(window.appNavigateTo) window.appNavigateTo('quiz-play');
        else console.error("Navigation function not found");
    },
    ...
    async updateRangeInputs() {
        ...
        try {
            ...
            const startStorageKey = LOCAL_STORAGE_KEYS.QUIZ_RANGE_START;
            const endStorageKey = LOCAL_STORAGE_KEYS.QUIZ_RANGE_END;
        ...
        if (newValueStr !== null && newValueStr.trim() !== '') {
            ...
            if (!isNaN(newValue)) {
                ...
                const storageKey = targetButton.id === 'quiz-range-start'
                                   ? LOCAL_STORAGE_KEYS.QUIZ_RANGE_START
                                   : LOCAL_STORAGE_KEYS.QUIZ_RANGE_END;
                try {
                ...
                ui.showToast("숫자만 입력 가능합니다.", true);
            }
        }
        this.hideRangeInput();
    },
    resetQuizRange() {
        ...
        try {
            localStorage.setItem(LOCAL_STORAGE_KEYS.QUIZ_RANGE_START, 1);
            localStorage.setItem(LOCAL_STORAGE_KEYS.QUIZ_RANGE_END, totalWords);
            this.clearAndPreloadQuizzesForNewRange();
        } catch (e) {
    ...
    async displayNextQuiz() {
        ...
        if (nextQuiz) {
            ...
            if (this.state.sessionAnsweredInSet > 0) {
                 ...
                 setTimeout(() => {
                     if(window.appNavigateTo) window.appNavigateTo('quiz');
                 }, 800);
            }
        }
    },
    ...
    reviewSessionMistakes() {
        ...
        this.state.sessionMistakes = [];
         api.syncOfflineData();
         if(window.appNavigateTo) window.appNavigateTo('mistakeReview', { mistakeWords: mistakes });
    },
    async preloadAllQuizTypesBasedOnSavedRange() {
        ...
        try {
            const savedStart = localStorage.getItem(LOCAL_STORAGE_KEYS.QUIZ_RANGE_START);
            const savedEnd = localStorage.getItem(LOCAL_STORAGE_KEYS.QUIZ_RANGE_END);
