// ================================================================
// ui-commons.js : 유틸리티, UI 헬퍼, 대시보드, 사운드
// ================================================================
...
function playSingleBeep({ frequency, duration = 0.1, type = 'sine', gain = 0.3, endFrequency }) {
    if (!app.state.audioContext || app.state.audioContext.state === 'suspended') {
         if(app.state.audioContext) app.state.audioContext.resume();
         else return;
    }
...
const correctBeep = { name: '또로롱', sequence: [{ frequency: 523, duration: 0.07, type: 'triangle', gain: 0.25 }, { delay: 80, frequency: 659, duration: 0.07, type: 'triangle', gain: 0.25 }, { delay: 160, frequency: 783, duration: 0.07, type: 'triangle', gain: 0.25 }] };
const incorrectBeep = { name: '삐빅', sequence: [{ frequency: 400, duration: 0.07, type: 'square', gain: 0.15 }, { delay: 90, frequency: 400, duration: 0.07, type: 'square', gain: 0.15 }] };
...
const utils = {
    ...
    isFavorite(word) {
        ...
        try {
            const key = app.state.LOCAL_STORAGE_KEYS.UNSYNCED_PROGRESS_UPDATES;
            const unsynced = JSON.parse(localStorage.getItem(key) || '{}');
            if (unsynced[word] && unsynced[word].favorite !== undefined) isFav = unsynced[word].favorite;
        } catch (e) {}
        return isFav;
    },
    getFavoriteWords() {
        ...
        combinedKeys.forEach(word => {
            ...
            const combinedState = { ...serverState, favorite: localState.favorite !== undefined ? localState.favorite : serverState.favorite, favoritedAt: localState.favoritedAt !== undefined ? localState.favoritedAt : serverState.favoritedAt };
            if (combinedState.favorite === true) favoriteWords.push({ word: word, time: combinedState.favoritedAt || 0 });
        });
...
const ui = {
    async copyToClipboard(text) {
        if (navigator.clipboard && text) { try { await navigator.clipboard.writeText(text); } catch (err) {} }
    },
    createInteractiveFragment(text, isForSampleSentence = false) {
        ...
        parts.forEach(part => {
            if (/([a-zA-Z0-9'-]+)/.test(part) && window.learningMode && learningMode.nonInteractiveWords && !learningMode.nonInteractiveWords.has(part.toLowerCase())) {
                ...
                span.oncontextmenu = (e) => { e.preventDefault(); if (isForSampleSentence) e.stopPropagation(); this.showWordContextMenu(e, part); };
                 let touchMove = false;
        ...
        text.split('\n').forEach((line, lineIndex, lineArr) => {
            ...
            while ((match = regex.exec(line))) {
                ...
                if (englishPhrase) {
                    ...
                    if (window.learningMode && !learningMode.nonInteractiveWords.has(englishPhrase.toLowerCase())) {
                        span.className = 'interactive-word';
                        span.onclick = () => { clearTimeout(app.state.longPressTimer); api.speak(englishPhrase, 'word'); };
                        span.oncontextmenu = (e) => { e.preventDefault(); this.showWordContextMenu(e, englishPhrase); };
                        let touchMove = false;
                        span.addEventListener('touchstart', (e) => {
                            touchMove = false; clearTimeout(app.state.longPressTimer);
                ...
                } else if (nonClickable) targetElement.appendChild(document.createTextNode(nonClickable));
        ...
        (sentences || []).filter(s => s && s.trim()).forEach(sentence => {
            ...
            p.addEventListener('mouseenter', (e) => {
                 if (e.target === p) {
                    ...
                    app.state.activeTranslationTarget = p;
                    app.state.translationTimer = setTimeout(() => { if (app.state.activeTranslationTarget === p) showTranslation(e); }, 1000);
                 }
            });
            p.addEventListener('mouseleave', () => {
                clearTimeout(app.state.translationTimer);
                if (app.state.activeTranslationTarget === p) app.state.activeTranslationTarget = null;
            ...
            sentenceParts.forEach(part => {
                ...
                } else if (part) { sentenceContent.appendChild(this.createInteractiveFragment(part, true)); }
            });
    ...
    hideTranslationTooltip() { app.elements.translationTooltip.classList.add('hidden'); },
    showWordContextMenu(event, word, options = {}) {
        ...
        menu.style.left = `0px`; menu.style.top = `${y}px`;
        ...
        requestAnimationFrame(() => {
            ...
            if (y + menuRect.height > window.innerHeight - 10) finalY = window.innerHeight - menuRect.height - 10;
             if (finalX < 10) finalX = 10; if (finalY < 10) finalY = 10;
        ...
        app.elements.searchDaumContextBtn.onclick = () => { window.open(`https://dic.daum.net/search.do?q=${encodedWord}`, 'dict_daum'); this.hideWordContextMenu(); };
        app.elements.searchNaverContextBtn.onclick = () => { window.open(`https://en.dict.naver.com/#/search?query=${encodedWord}`, 'dict_naver'); this.hideWordContextMenu(); };
        app.elements.searchEtymContextBtn.onclick = () => { window.open(`https://www.etymonline.com/search?q=${encodedWord}`, 'dict_etym'); this.hideWordContextMenu(); };
        app.elements.searchLongmanContextBtn.onclick = () => { window.open(`https://www.ldoceonline.com/dictionary/${encodedWord}`, 'dict_longman'); this.hideWordContextMenu(); };
        // Click outside listener is in core.js global binding
    },
    hideWordContextMenu() { if (app.elements.wordContextMenu) app.elements.wordContextMenu.classList.add('hidden'); }
};
...
const dashboard = {
    ...
    state: { studyTimeChart: null, quiz1Chart: null, quiz2Chart: null, quiz3Chart: null },
    ...
    destroyCharts() {
        ...
        this.state.studyTimeChart = null; this.state.quiz1Chart = null; this.state.quiz2Chart = null; this.state.quiz3Chart = null;
    },
    async render() {
        if (!app.state.isWordListReady) { this.elements.content.innerHTML = `<div class="text-center p-10"><p class="text-gray-600">단어 목록을 먼저 불러와주세요.</p></div>`; return; }
        const wordList = app.state.wordList;
        const totalWords = wordList.length;
        const stages = { unseen: { name: '새 단어', count: 0, color: 'bg-gray-400' }, learning: { name: '학습 중', count: 0, color: 'bg-blue-500' }, review: { name: '복습 필요', count: 0, color: 'bg-orange-500' }, learned: { name: '학습 완료', count: 0, color: 'bg-green-500' } };
        wordList.forEach(wordObj => { const status = utils.getWordStatus(wordObj.word); if (stages[status]) stages[status].count++; });
    ...
    async renderSummary() {
        ...
        for (let i = 6; i >= 0; i--) { const d = new Date(today); d.setDate(d.getDate() - i); labels.push(`${d.getMonth() + 1}/${d.getDate()}`); data.push(Math.round((studyHistory[d.toISOString().slice(0, 10)] || 0) / 60)); }
        const studyTimeCtx = document.getElementById('study-time-chart')?.getContext('2d');
        if (studyTimeCtx) { this.state.studyTimeChart = new Chart(studyTimeCtx, { type: 'bar', data: { labels: labels, datasets: [{ label: '학습 시간 (분)', data: data, backgroundColor: 'rgba(54, 162, 235, 0.6)', borderColor: 'rgba(54, 162, 235, 1)', borderWidth: 1 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, suggestedMax: 60 } }, plugins: { legend: { display: false } } } }); }

        const totalQuizStats = { 'MULTIPLE_CHOICE_MEANING': { correct: 0, total: 0 }, 'FILL_IN_THE_BLANK': { correct: 0, total: 0 }, 'MULTIPLE_CHOICE_DEFINITION': { correct: 0, total: 0 } };
        for (let i = 0; i < 7; i++) { const d = new Date(today); d.setDate(d.getDate() - i); const dateString = d.toISOString().slice(0, 10); if (quizHistory[dateString]) { for (const type in totalQuizStats) { if (quizHistory[dateString][type]) { totalQuizStats[type].correct += quizHistory[dateString][type].correct || 0; totalQuizStats[type].total += quizHistory[dateString][type].total || 0; } } } }

        const createDoughnutChart = (elementId, labelId, labelText, stats) => {
            const ctx = document.getElementById(elementId)?.getContext('2d'); if (!ctx) return null;
            const correct = stats.correct || 0; const total = stats.total || 0; const incorrect = total - correct; const accuracy = total > 0 ? ((correct / total) * 100).toFixed(0) : 0;
            const labelEl = document.getElementById(labelId); if (labelEl) labelEl.textContent = `${labelText} (${correct}/${total})`;
            return new Chart(ctx, { type: 'doughnut', data: { labels: total > 0 ? ['정답', '오답'] : ['기록 없음'], datasets: [{ data: total > 0 ? [correct, incorrect > 0 ? incorrect : 0.0001] : [0, 1], backgroundColor: total > 0 ? ['#34D399', '#F87171'] : ['#E5E7EB', '#E5E7EB'], hoverBackgroundColor: total > 0 ? ['#10B981', '#EF4444'] : ['#D1D5DB', '#D1D5DB'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: true, cutout: '70%', plugins: { legend: { display: false }, tooltip: { enabled: false } } }, plugins: [{ id: 'doughnutLabel', beforeDraw: (chart) => { const { ctx, width, height } = chart; ctx.restore(); const fontSize = (height / 100).toFixed(2); ctx.font = `bold ${fontSize}em sans-serif`; ctx.textBaseline = 'middle'; const text = total > 0 ? `${accuracy}%` : '-'; const textX = Math.round((width - ctx.measureText(text).width) / 2); const textY = height / 2; ctx.fillStyle = total > 0 ? '#374151' : '#9CA3AF'; ctx.fillText(text, textX, textY); ctx.save(); } }] });
        };
        ...
        if (textSummaryContainer) {
            const getStatsForPeriod = (days) => {
                let totalSeconds = 0; const quizStats = { 'MULTIPLE_CHOICE_MEANING': { correct: 0, total: 0 }, 'FILL_IN_THE_BLANK': { correct: 0, total: 0 }, 'MULTIPLE_CHOICE_DEFINITION': { correct: 0, total: 0 } };
                for (let i = 0; i < days; i++) { const d = new Date(today); d.setDate(d.getDate() - i); const dateString = d.toISOString().slice(0, 10); totalSeconds += studyHistory[dateString] || 0; if (quizHistory[dateString]) { for (const type in quizStats) { if(quizHistory[dateString][type]) { quizStats[type].correct += quizHistory[dateString][type].correct || 0; quizStats[type].total += quizHistory[dateString][type].total || 0; } } } }
            ...
            const quizHistoryTotal = { 'MULTIPLE_CHOICE_MEANING': { correct: 0, total: 0 }, 'FILL_IN_THE_BLANK': { correct: 0, total: 0 }, 'MULTIPLE_CHOICE_DEFINITION': { correct: 0, total: 0 } };
            if(quizHistory) { Object.values(quizHistory).forEach(daily => { Object.entries(daily).forEach(([type, stats]) => { if (quizHistoryTotal[type] && stats) { quizHistoryTotal[type].correct += stats.correct || 0; quizHistoryTotal[type].total += stats.total || 0; } }); }); }
            const stats30 = getStatsForPeriod(30);
            const createSummaryCardHTML = (title, totalSeconds, quizStats) => {
                const quizTypes = { 'MULTIPLE_CHOICE_MEANING': '영한 뜻', 'FILL_IN_THE_BLANK': '빈칸 추론', 'MULTIPLE_CHOICE_DEFINITION': '영영 풀이' };
                let quizHTML = '<div class="grid grid-cols-3 gap-1 text-center">';
                for (const type in quizTypes) { const stats = quizStats[type] || { correct: 0, total: 0 }; const accuracy = stats.total > 0 ? ((stats.correct / stats.total) * 100).toFixed(0) : 0; quizHTML += `<div class="bg-white p-2 rounded-lg shadow-sm"><p class="text-sm font-semibold text-gray-500">${quizTypes[type]}</p><p class="font-bold text-gray-800 text-xl">${accuracy}%</p><p class="text-xs text-gray-400">(${stats.correct}/${stats.total})</p></div>`; }
                quizHTML += '</div>';
                return `<div class="bg-gray-50 p-4 rounded-xl shadow-inner"><h4 class="font-bold text-gray-700 mb-4 text-lg text-center">${title} <span class="font-normal text-gray-500">(${utils.formatSeconds(totalSeconds)})</span></h4><div class="space-y-3">${quizHTML}</div></div>`;
            };
            textSummaryContainer.innerHTML = `<div class="space-y-6">${createSummaryCardHTML('최근 30일 기록', stats30.totalSeconds, stats30.quizStats)}${createSummaryCardHTML('누적 총학습 기록', totalStudySeconds, quizHistoryTotal)}</div>`;
        }
    }
};