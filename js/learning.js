...
export const learningMode = {
  ...
  handleCardContextMenu(e) {
    ...
    if (e.target.closest('#word-header') || e.target.closest('#meaning-container') ||
        e.target.closest('#explanation-container') || e.target.closest('#learning-card-back')) return;
    e.preventDefault();
  ...
  async enterEditMode(side) {
    ...
    if (side === 'front') {
      ...
      this.elements.wordDisplay.innerHTML = `
        <div class="flex flex-col gap-2">
          ...
          <button id="auto-fill-btn"
            class="self-center text-xs bg-indigo-100 hover:bg-indigo-200 text-indigo-700 py-1 px-3 rounded-full transition-colors mb-2 font-semibold shadow-sm flex items-center gap-1">
            🤖 AI 자동입력
          </button>
        </div>`;
      ...
      this.elements.meaningDisplay.innerHTML = `
        <div id="edit-meaning-input" contenteditable="true"
      ...
      this.elements.explanationDisplay.innerHTML = `
        <div id="edit-explanation-input" contenteditable="true"
          class="select-text w-full p-2 border border-blue-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white cursor-text"
      ...
      setTimeout(() => {
        ...
        richEditors.forEach(editor => {
          ...
          editor.addEventListener('input', () => { editor.value = editor.innerHTML; });
          ...
          const showFormatTooltip = () => {
            ...
            tooltip.style.cssText = `position:fixed;z-index:10000;top:${rect.top - 50}px;left:${rect.left}px`;
            const actions = [
          ...
          editor.addEventListener('keyup', e => { if (e.shiftKey) setTimeout(showFormatTooltip, 10); });
        ...
        // #9 fix: store handler reference for proper cleanup
        this._mousedownCleanupHandler = e => {
          const tooltip = document.getElementById('format-tooltip');
          const isEditorClick = richEditors.some(ed => ed && ed.contains(e.target));
          if (tooltip && !tooltip.contains(e.target) && !isEditorClick) tooltip.remove();
        };
        document.addEventListener('mousedown', this._mousedownCleanupHandler);
        ...
        if (autoBtn) {
          ...
          autoBtn.onclick = async () => {
            let targetWord = wordInput.value.trim().replace(/\[.*?\]/, '').trim();
            if (!targetWord) {
              window.dispatchEvent(new CustomEvent('showToast', { detail: { message: '단어를 입력해주세요.', isError: true } }));
              return;
            }
            ...
            try {
              ...
              if (aiData.samples && aiData.samples.length > 0) {
                ...
                window.dispatchEvent(new CustomEvent('showToast', { detail: { message: 'AI 정보를 가져왔습니다.' } }));
              }
            } catch(e) {
              console.error(e);
              window.dispatchEvent(new CustomEvent('showToast', { detail: { message: 'AI 자동입력 실패', isError: true } }));
            } finally {
              autoBtn.disabled = false;
              autoBtn.innerHTML = '🤖 AI 자동입력';
            }
          };
        }
      }, 0);
      ...
      const currentSample = wordData.sample || '';
      this.elements.backContent.innerHTML = `<textarea id="edit-sample-input"
        class="w-full h-full p-2 border border-blue-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
        rows="10" placeholder="예문을 입력하세요...">${currentSample}</textarea>`;
      const aiSection = this.elements.backContent.parentNode.querySelector('.ai-gen-section');
      if (aiSection) aiSection.style.display = 'none';
      this.elements.sampleBtnImg.src = 'images/cat-edit.png';
    }
  },

  async saveAndExitEditMode() {
    // #9 fix: remove stored mousedown listener
    if (this._mousedownCleanupHandler) {
      document.removeEventListener('mousedown', this._mousedownCleanupHandler);
      this._mousedownCleanupHandler = null;
    }
    const tooltip = document.getElementById('format-tooltip');
    ...
    if (side === 'front') {
      ...
      if (wordInput && meaningInput && explanationInput) {
        ...
        let newPos;
        if (match) { newWord = match[1].trim(); newPos = match[2].trim(); }
        else if (wordData.isNew) { newPos = ''; }
        ...
        if (newWord !== wordData.word) {
          const isDuplicate = state.wordList.some(w =>
            w.word.toLowerCase() === newWord.toLowerCase() && w !== wordData
          );
          if (isDuplicate) {
        ...
        if (wordData.isNew) {
          const newCardData = { word: newWord, pos: newPos || '', meaning: newMeaning, explanation: newExplanation, manualsample: newSample };
          let afterWord = null;
          if (this.state.currentIndex > 0) afterWord = this.state.currentWordList[this.state.currentIndex - 1].word;
          await api.createWord(newCardData, afterWord);
          wordData.word = newWord; wordData.pos = newPos; wordData.meaning = newMeaning;
          wordData.explanation = newExplanation; wordData.sample = newSample;
          delete wordData.isNew;
          window.dispatchEvent(new CustomEvent('showToast', { detail: { message: '새 단어가 추가되었습니다.' } }));
        } else {
          await api.updateWordDetails(wordData.word, { word: newWord, pos: newPos, meaning: newMeaning, explanation: newExplanation, manualsample: newSample });
          wordData.word = newWord;
          if (newPos !== undefined) wordData.pos = newPos;
          wordData.meaning = newMeaning; wordData.explanation = newExplanation; wordData.sample = newSample;
        }
      }
    } else {
      ...
      if (sampleInput) {
        ...
        if (wordData.isNew) {
          if (!wordData.word) {
            window.dispatchEvent(new CustomEvent('showToast', { detail: { message: '먼저 단어를 입력해주세요.', isError: true } }));
            return;
          }
          wordData.sample = newSampleText;
        } else {
          await api.updateWordDetails(wordData.word, { manualsample: newSampleText });
          wordData.sample = newSampleText;
        }
        const aiSection = this.elements.backContent.parentNode.querySelector('.ai-gen-section');
  ...
  async createNewCard() {
    const tempCard = { word: '', pos: '', meaning: '', explanation: '', sample: '', isNew: true };
    ...
    setTimeout(() => { this.state.editingSide = 'front'; this.enterEditMode('front'); }, 100);
  },
  ...
  async deleteCurrentCard() {
    ...
    if (wordData.isNew) {
      ...
      if (this.state.currentWordList.length === 0) this.reset();
      else this.displayWord(this.state.currentIndex, true);
      return;
    }
    ...
    window.dispatchEvent(new CustomEvent('showToast', { detail: { message: '단어가 삭제되었습니다.' } }));
  },
  ...
  async start() {
    ...
    if (!state.isWordListReady) {
      this.elements.loaderText.textContent = '단어 목록 불러오는 중...';
      try {
        await api.loadWordList();
        await api.loadUserProgress();
      } catch(e) {
        this.showError('단어 목록을 불러오지 못했습니다.', e.message);
        return;
      }
    }

    this.state.currentWordList = state.wordList;
    if (this.state.currentWordList.length === 0) { this.showError('단어 목록이 비어있습니다.', ''); return; }

    const startWord = this.elements.startWordInput.value.trim();
    if (!startWord) {
      try {
        const savedIndex = parseInt(localStorage.getItem(state.LOCALSTORAGEKEYS.LASTINDEX) || '0');
    ...
    const exactMatches = [], startsWithMatches = [], includesMatches = [], fuzzyMatches = [], explanationMatches = [];
    const searchRegex = new RegExp(lowerCaseStartWord.replace(/[-?]/g, '.'), 'i');
    const limit = 2;
    ...
    for (let index = 0; index < this.state.currentWordList.length; index++) {
      ...
      if (wordLower === lowerCaseStartWord) exactMatches.push(wordObj);
      else if (wordLower.startsWith(lowerCaseStartWord)) startsWithMatches.push(wordObj);
      else if (wordLower.includes(lowerCaseStartWord)) includesMatches.push(wordObj);
      else {
        const lenDiff = Math.abs(wordLower.length - lowerCaseStartWord.length);
        if (lenDiff <= limit) {
          const dist = utils.levenshteinDistance(lowerCaseStartWord, wordLower, limit);
        ...
        if (item.explanation) {
          ...
          if (searchRegex.test(cleanedExplanation)) explanationMatches.push({ word: item.word, index });
        }
      }
    }

    fuzzyMatches.sort((a, b) => a.distance - b.distance);
    const vocabSuggestions = [...exactMatches, ...startsWithMatches, ...includesMatches, ...fuzzyMatches].slice(0, 50);

    if (exactMatches.length === 1 && startsWithMatches.length === 0 && includesMatches.length === 0) {
      this.state.currentIndex = exactMatches[0].index;
      this.launchApp();
      return;
    }
    ...
    if (vocabSuggestions.length === 0 && explanationMatches.length === 0)
      title = `<strong>${startWord}</strong>에 대한 결과가 없습니다.`;
  ...
  showError(message, detail) {
    ...
    this.elements.loaderText.innerHTML = `<p class="text-red-500 font-bold">${message}</p>${detail ? `<p class="text-sm text-gray-600 mt-2 break-all">${detail}</p>` : ''}`;
  },
  ...
  displayWord(index, silent = false) {
    this.state.isEditing = false;
    ...
    if (!this.state.isMistakeMode && !this.state.isFavoriteMode) {
      try { localStorage.setItem(state.LOCALSTORAGEKEYS.LASTINDEX, index); } catch(e) {}
    }
    ...
    this.elements.sampleBtnImg.src = hasSample ? 'images/cat-delivery.png' : 'images/cat-add.png';
  ...
  navigate(direction) {
    ...
    const navigateAction = () => {
      ...
      this.displayWord(this.state.currentIndex);
      if (isBackVisible) this.handleFlip();
    };
    if (isBackVisible) setTimeout(navigateAction, 300);
    else navigateAction();
  },

  // #27 fix: _renderBackContent helper extracted
  _renderBackContent(wordData) {
    ...
    if (wordData.sample && wordData.sample.trim()) {
      ...
      this.elements.backContent.innerHTML =
        '<div class="flex h-full items-center justify-center text-gray-400">예문 없음<br>📝 길게 눌러 편집</div>';
  ...
  async navigateBackToBack(direction) {
    ...
    this._renderBackContent(wordData);   // #27 fix
  ...
  async handleFlip() {
    ...
    if (!isBackVisible) {
      ...
      this.elements.cardBack.classList.remove('is-slid-up');
      const hasSample = (wordData.sample && wordData.sample.trim() !== '') ||
        (wordData.AISample && wordData.AISample.en);
      this.elements.sampleBtnImg.src = hasSample ? 'images/cat-delivery.png' : 'images/cat-add.png';
    }
  },
  ...
  startMistakeReview(mistakeWords) {
    ...
    this.state.isFavoriteMode = false;
    const wordMap = new Map(state.wordList.map(w => [w.word, w]));
    this.state.currentWordList = mistakeWords.map(word => wordMap.get(word)).filter(Boolean);
    this.state.currentIndex = 0;
    if (this.state.currentWordList.length === 0) {
      window.dispatchEvent(new CustomEvent('showToast', { detail: { message: '복습할 단어가 없습니다.', isError: true } }));
  ...
  async startFavoriteMode() {
    ...
    if (favoriteWords.length === 0) {
      window.dispatchEvent(new CustomEvent('showToast', { detail: { message: '즐겨찾기한 단어가 없습니다.', isError: true } }));
      window.dispatchEvent(new CustomEvent('navigate', { detail: { mode: 'selection' } }));
      return;
    }
    const wordMap = new Map(state.wordList.map(w => [w.word, w]));
  ...
  handleTouchStart(e) {
    if (this.elements.appContainer.classList.contains('hidden')) return;
    if (e.target.closest('button, a, input, [onclick], #progress-bar-track')) return;
    if (this.state.isEditing) return;
  ...
  handleTouchEnd(e) {
    if (this.elements.appContainer.classList.contains('hidden') ||
        ...
        e.target.closest('button, a, input, [onclick], #progress-bar-track')) {
  ...
  renderInitialGenButton(container, wordData) {
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        if (!newSentences || newSentences.length === 0) throw new Error('예문 없음');
    ...
    botBtn.onclick = async e => {
      ...
      try {
        ...
        if (newSentence && newSentence.length > 0) {
          allSentences[index] = newSentence[0];
